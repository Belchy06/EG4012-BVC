...
namespace bvc_vector
{

	//===========================================================================
	// This header contains a definition for all the most common mathematical
	// operations on vectors implemented as overloading of standard operators.
	//===========================================================================
	...
	// Lp-norm of vector
	template <typename T>
	T lp_norm(const std::vector<T>& vector, T p)
	{
	...
	typedef enum
	{
		BVC_WAVELET_CONVOLUTION_FULL,
		BVC_WAVELET_CONVOLUTION_SAME,
		BVC_WAVELET_CONVOLUTION_VALID,
	} bvc_wavelet_convolution_type;

	// Convolution of vector with given filter
	template <typename T>
	std::vector<T> conv(const std::vector<T>& vector, const std::vector<T>& filter, bvc_wavelet_convolution_type convolutionType = BVC_WAVELET_CONVOLUTION_FULL)
	{
		...
		if (convolutionType == BVC_WAVELET_CONVOLUTION_SAME)
		{
			minI = filter_size / 2;
			maxI = minI + vector_size;
		}
		else if (convolutionType == BVC_WAVELET_CONVOLUTION_VALID)
		{
			minI = filter_size - 1;
			maxI = vector_size;
		}
