...
std::shared_ptr<wavelet_recomposer> wavelet_recomposer_factory::create_wavelet_recomposer(ovc::wavelet_family in_wavelet_transform, ovc::wavelet_config in_config)
{
	switch (in_wavelet_transform)
	{
		case ovc::WAVELET_FAMILY_BIORTHOGONAL:
			return std::make_shared<biorthogonal_recomposer>(in_config);
		case ovc::WAVELET_FAMILY_COIFLETS:
			return std::make_shared<coiflets_recomposer>(in_config);
		case ovc::WAVELET_FAMILY_DAUBECHIES:
			return std::make_shared<daubechies_recomposer>(in_config);
		case ovc::WAVELET_FAMILY_HAAR:
			return std::make_shared<haar_recomposer>(in_config);
		case ovc::WAVELET_FAMILY_REVERSE_BIORTHOGONAL:
			return std::make_shared<reverse_biorthogonal_recomposer>(in_config);
		case ovc::WAVELET_FAMILY_SKIP:
			return std::make_shared<skip_recomposer>(in_config);
		case ovc::WAVELET_FAMILY_SYMLETS:
