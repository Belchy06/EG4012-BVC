...
typedef enum
{
	OVC_WAVELET_FAMILY_SKIP,
	OVC_WAVELET_FAMILY_BIORTHOGONAL,
	OVC_WAVELET_FAMILY_COIFLETS,
	OVC_WAVELET_FAMILY_DAUBECHIES,
	OVC_WAVELET_FAMILY_HAAR,
...
typedef enum
{
	OVC_WAVELET_SUBBAND_DIAGONAL,
	OVC_WAVELET_SUBBAND_VERTICAL,
	OVC_WAVELET_SUBBAND_HORIZONTAL
} ovc_wavelet_subbdand;

template <typename T>
class ovc_wavelet_decomposition_1d : public std::vector<std::vector<T>>
{
public:
	explicit ovc_wavelet_decomposition_1d(size_t in_num_levels);
...
template <typename T>
class ovc_wavelet_decomposition_2d : public std::vector<matrix<T>>
{
public:
	explicit ovc_wavelet_decomposition_2d(size_t in_num_levels);
	~ovc_wavelet_decomposition_2d();

	size_t num_levels() const;

	const matrix<T>&					   get_app_coefficients() const;
	const matrix<T>&					   get_det_coefficients(ovc_wavelet_subbdand in_subband, size_t in_level) const;
	matrix<T>							   to_matrix();
	static ovc_wavelet_decomposition_2d<T> from_matrix(matrix<T> in_x, size_t in_num_levels)
	{
	...
	void set_det_coefficients(const matrix<T>& in_d, ovc_wavelet_subbdand in_subband, size_t in_level);
...
public:
	...
	ovc_wavelet_decomposition_1d<T> decompose(const std::vector<T>& in_x, size_t in_num_levels) const;

	// Multi-level wavelet reconstruction of 1D vector having length_rec
	std::vector<T> reconstruct(const ovc_wavelet_decomposition_1d<T>& in_decomposition, size_t in_length_rec) const;
	...
	ovc_wavelet_decomposition_2d<T> decompose(const matrix<T>& in_x, size_t in_num_levels) const;

	// Multi-level wavelet reconstruction of 2D matrix having size_rec
	matrix<T> reconstruct(const ovc_wavelet_decomposition_2d<T>& in_decomposition, const matrix_size& in_size_rec) const;
...
private:
	...
	// the reconstructed vector is inferred by the input ovc_wavelet_decomposition_1d and it
	...
	std::vector<T> app_coefficients(const ovc_wavelet_decomposition_1d<T>& decomposition, size_t level) const;

	// Reconstruct the vector approximation at given level and specify the length
	// of the reconstruction using the third argument
	std::vector<T> app_coefficients(const ovc_wavelet_decomposition_1d<T>& decomposition, size_t level, size_t length_rec) const;
	...
	matrix<T> app_coefficients(const ovc_wavelet_decomposition_2d<T>& decomposition, size_t level) const;

	// As above, but with the extra argument specifying the size of the matrix
	matrix<T> app_coefficients(const ovc_wavelet_decomposition_2d<T>& decomposition, size_t level, const matrix_size& size_rec) const;
