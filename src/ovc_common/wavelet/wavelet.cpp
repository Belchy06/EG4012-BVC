...
namespace ovc
{
	...
	template <typename T>
	wavelet_decomposition_1d<T>::wavelet_decomposition_1d(size_t in_num_levels)
	{
	...
	template <typename T>
	wavelet_decomposition_1d<T>::~wavelet_decomposition_1d() {}

	template <typename T>
	size_t wavelet_decomposition_1d<T>::num_levels() const
	{
		return this->size() - 1;
	}

	template <typename T>
	const std::vector<T>& wavelet_decomposition_1d<T>::get_app_coefficients() const
	{
		return this->back();
	}

	template <typename T>
	const std::vector<T>& wavelet_decomposition_1d<T>::get_det_coefficients(size_t in_level) const
	{
		if (in_level >= num_levels())
		{
			throw std::invalid_argument("wavelet_decomposition_1d::get_det_coefficients");
		}
		return this->at(in_level);
	}

	template <typename T>
	void wavelet_decomposition_1d<T>::set_det_coefficients(const std::vector<T>& in_d, size_t in_level)
	{
		if (in_level >= num_levels())
		{
			throw std::invalid_argument("wavelet_decomposition_1d::set_det_coefficients");
		}
		this->operator[](in_level) = in_d;
	}

	template <typename T>
	void wavelet_decomposition_1d<T>::set_app_coefficients(const std::vector<T>& in_a)
	{
		this->back() = in_a;
	}
	...
	template <typename T>
	wavelet_decomposition_2d<T>::wavelet_decomposition_2d(size_t in_num_levels)
	{
		// Internal representation is structured as
	...
	template <typename T>
	wavelet_decomposition_2d<T>::~wavelet_decomposition_2d() {}

	template <typename T>
	size_t wavelet_decomposition_2d<T>::num_levels() const
	{
		return (this->size() - 1) / 3;
	}

	template <typename T>
	void wavelet_decomposition_2d<T>::set_app_coefficients(const matrix<T>& in_a)
	{
		this->back() = in_a;
	}

	template <typename T>
	const matrix<T>& wavelet_decomposition_2d<T>::get_app_coefficients() const
	{
		return this->back();
	}

	template <typename T>
	const matrix<T>& wavelet_decomposition_2d<T>::get_det_coefficients(wavelet_subbdand in_subband, size_t in_n) const
	{
		if (in_subband != WAVELET_SUBBAND_HORIZONTAL && in_subband != WAVELET_SUBBAND_VERTICAL && in_subband != WAVELET_SUBBAND_DIAGONAL)
		{
			throw std::invalid_argument("wavelet_decomposition_2d::get_det_coefficients");
		}
		if (in_n >= num_levels())
		{
			throw std::invalid_argument("wavelet_decomposition_2d::get_det_coefficients ");
	...
	template <typename T>
	void wavelet_decomposition_2d<T>::set_det_coefficients(const matrix<T>& in_d, wavelet_subbdand in_subband, size_t in_n)
	{
		if (in_subband != WAVELET_SUBBAND_HORIZONTAL && in_subband != WAVELET_SUBBAND_VERTICAL && in_subband != WAVELET_SUBBAND_DIAGONAL)
		{
			throw std::invalid_argument("wavelet_decomposition_2d::set_det_coefficients");
		}
		if (in_n >= num_levels())
		{
			throw std::invalid_argument("wavelet_decomposition_2d::set_det_coefficients ");
	...
	template <typename T>
	matrix<T> wavelet_decomposition_2d<T>::to_matrix()
	{
		matrix<T> full;
	...
	template <typename T>
	wavelet_decomposition_1d<T> wavelet<T>::decompose(const std::vector<T>& in_x, size_t in_num_levels) const
	{
		if (in_x.size() < (size_t)(1 << in_num_levels))
		{
			throw std::invalid_argument("wavelet::decompose");
		}
		wavelet_decomposition_1d<T> decomposition(in_num_levels);
		std::vector<T>				a = in_x;
	...
	template <typename T>
	std::vector<T> wavelet<T>::reconstruct(const wavelet_decomposition_1d<T>& in_decomposition, size_t in_length_rec) const
	{
	...
	template <typename T>
	wavelet_decomposition_2d<T> wavelet<T>::decompose(const matrix<T>& in_x, size_t in_num_levels) const
	{
	...
	template <typename T>
	matrix<T> wavelet<T>::reconstruct(const wavelet_decomposition_2d<T>& decomposition, const matrix_size& size_rec) const
	{
	...
	{
		wavelet_decomposition_1d<T> c(num_levels);
	...
	{
		wavelet_decomposition_2d<T> c(num_levels);
		...
		for (size_t k = 0; k < num_levels; k++)
		{
			...
			c.set_det_coefficients(tmp, WAVELET_SUBBAND_DIAGONAL, k);
			c.set_det_coefficients(tmp, WAVELET_SUBBAND_VERTICAL, k);
			c.set_det_coefficients(tmp, WAVELET_SUBBAND_HORIZONTAL, k);
		}
	...
	template <typename T>
	std::vector<T> wavelet<T>::app_coefficients(const wavelet_decomposition_1d<T>& decomposition, size_t level) const
	{
	...
	template <typename T>
	std::vector<T> wavelet<T>::app_coefficients(const wavelet_decomposition_1d<T>& decomposition, size_t level, size_t length_rec) const
	{
	...
	std::vector<T> wavelet<T>::conv_down(const std::vector<T>& x, const std::vector<T>& f) const
	{
		...
		z = conv(z, f, WAVELET_CONVOLUTION_VALID);
	...
	std::vector<T> wavelet<T>::up_conv(const std::vector<T>& x, const std::vector<T>& f, size_t s) const
	{
		...
		y = w_extend(y, lf / 2);
		y = conv(y, f, WAVELET_CONVOLUTION_FULL);
	...
	template <typename T>
	matrix<T> wavelet<T>::app_coefficients(const wavelet_decomposition_2d<T>& decomposition, size_t level) const
	{
		if (level >= decomposition.num_levels())
		{
		...
		if (level == 0)
		{
			...
			const matrix_size size_level_0 = decomposition.get_det_coefficients(WAVELET_SUBBAND_HORIZONTAL, 0).size();
		...
		else
		{
			...
			size_rec = decomposition.get_det_coefficients(WAVELET_SUBBAND_HORIZONTAL, level - 1).size();
		}
		return app_coefficients(decomposition, level, size_rec);
	}

	template <typename T>
	matrix<T> wavelet<T>::app_coefficients(const wavelet_decomposition_2d<T>& decomposition, size_t level,
		const matrix_size& size_rec) const
	{
		...
		for (size_t k = decomposition.num_levels(); k != level; k--)
		{
			...
			const matrix_size s = (k != 1) ? decomposition.get_det_coefficients(WAVELET_SUBBAND_HORIZONTAL, k - 2).size() : size_rec;
			const matrix<T>&  hk = decomposition.get_det_coefficients(WAVELET_SUBBAND_HORIZONTAL, k - 1);
			const matrix<T>&  vk = decomposition.get_det_coefficients(WAVELET_SUBBAND_VERTICAL, k - 1);
			const matrix<T>&  dk = decomposition.get_det_coefficients(WAVELET_SUBBAND_DIAGONAL, k - 1);
	...
	template class wavelet_decomposition_1d<double>;
	template class wavelet_decomposition_2d<double>;
