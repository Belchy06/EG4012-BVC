...
void ovc_bitstream::write_bit(uint8_t in_bit)
{
	in_bit = in_bit & 0x1;
...
void ovc_bitstream::write_byte(uint8_t in_byte)
{
...
bool ovc_bitstream::read_bit(uint8_t* out_bit)
{
...
bool ovc_bitstream::read_byte(uint8_t* out_byte)
{
	return true;
}

uint8_t* ovc_bitstream::data()
{
	return stream.data();
}

uint32_t ovc_bitstream::size()
{
	return write_idx - read_idx;
}

uint32_t ovc_bitstream::occupancy()
{
	return align(write_idx - read_idx, 8) >> 3;
}

void ovc_bitstream::clear()
{
	write_idx = 0;
	read_idx = 0;
...
uint32_t ovc_bitstream::align(uint32_t in_value, uint32_t in_alignment)
{
