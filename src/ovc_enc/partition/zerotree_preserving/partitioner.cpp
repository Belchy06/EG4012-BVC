...
std::vector<matrix<double>> zerotree_preserving_partitioner::partition(const matrix<double>& in_matrix, size_t in_num_levels, size_t in_num_parts)
{
	...
	{
		matrix<double> partition = matrix<double>((size_t)(2 * in_matrix.get_num_rows() / in_num_parts), (size_t)(2 * in_matrix.get_num_columns() / in_num_parts));
		...
		for (int64_t l = in_num_levels - 1; l >= 0; l--)
		{
			...
			size_t x_samples_per_level = 2 * mat_x / (size_t)in_num_parts;
			size_t y_samples_per_level = 2 * mat_y / (size_t)in_num_parts;
			...
			for (size_t y = 0; y < y_samples_per_level; y++)
			{
				for (size_t x = 0; x < x_samples_per_level; x++)
				{
					...
					partition(x, y) = in_matrix((x + offset_x) * step_x, (y + offset_y) * step_y);
				}
			}
		}
